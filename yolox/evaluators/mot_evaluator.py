...
class MOTEvaluator:
    ...
    def _get_color(track_id):
        r = (track_id * 37 + 100) % 256
        g = (track_id * 97 + 50) % 256
        b = (track_id * 137 + 150) % 256
        ...
        for cur_iter, (imgs, _, info_imgs, ids) in enumerate(
            ...
            if video_folder is not None and vid_img_dir is not None:
                ...
                if video_writer is None:
                    orig_h = int(info_imgs[0].item())
                    orig_w = int(info_imgs[1].item())
                    vpath = os.path.join(video_folder, '{}.mp4'.format(video_name))
                    fourcc = cv2.VideoWriter_fourcc(*'mp4v')
                    video_writer = cv2.VideoWriter(vpath, fourcc, 30, (orig_w, orig_h))
                img_path = os.path.join(vid_img_dir, vid_img_name, img_file_name[0])
    ...
    def evaluate_lstm(
        ...
        from new_pipeline.lstm import BYTETrackerLSTM
        ...
        tracker = BYTETrackerLSTM(
            self.args,
            lstm_ckpt=self.args.lstm_ckpt,
            assoc_ckpt=getattr(self.args, "assoc_ckpt", None),
            assoc_weight=getattr(self.args, "assoc_weight", 0.35),
            assoc_seq_len=getattr(self.args, "assoc_seq_len", 16),
            assoc_hidden_size=getattr(self.args, "assoc_hidden_size", 128),
            assoc_num_layers=getattr(self.args, "assoc_num_layers", 1),
            assoc_dropout=getattr(self.args, "assoc_dropout", 0.1),
            assoc_mlp_hidden=getattr(self.args, "assoc_mlp_hidden", 128),
            assoc_min_history=getattr(self.args, "assoc_min_history", 2),
        )
        ori_thresh = self.args.track_thresh
        ...
        for cur_iter, (imgs, _, info_imgs, ids) in enumerate(
            ...
            with torch.no_grad():
                ...
                if video_name not in video_names:
                    video_names[video_id] = video_name
                if frame_id == 1:
                    tracker = BYTETrackerLSTM(self.args, lstm_ckpt=self.args.lstm_ckpt)
                    if len(results) != 0:
                        result_filename = os.path.join(result_folder, '{}.txt'.format(video_names[video_id - 1]))
                        write_results(result_filename, results)
                        results = []
    ...
    def evaluate_xlstm(
        ...
        from new_pipeline.xlstm import BYTETrackerSLSTM
        ...
        tracker = BYTETrackerSLSTM(self.args, slstm_ckpt=self.args.xlstm_ckpt)
        ori_thresh = self.args.track_thresh
