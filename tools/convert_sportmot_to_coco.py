...
def list_image_files(img_path):
    return sorted([
        image for image in os.listdir(img_path)
        if image.lower().endswith(".jpg") and not image.startswith(".")
    ])
...
for split in SPLITS:
    ...
    if not os.path.isdir(data_path):
        print("Skip missing split: {}".format(data_path))
        continue
    ...
    for seq in tqdm(sorted(video_list)):
        ...
        images = list_image_files(img_path)
        seq_length = read_seq_length(seq_path)
        num_images = seq_length if seq_length is not None else len(images)
        ...
        for i in range(num_images):
            ...
            if img is None:
                raise FileNotFoundError(
                    "Cannot read image for sequence {} frame {}. "
                    "Expected: {}".format(
                        seq, i + 1,
                        os.path.join(data_path,
                                     "{}/img1/{:06d}.jpg".format(seq, i + 1))))
