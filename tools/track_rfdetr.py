...
def make_parser():
    parser = argparse.ArgumentParser("RF-DETR + ByteTrack Eval")
    ...
    parser.add_argument(
        ...
        default="pretrained",
        type=str,
        help="RF-DETR checkpoint file, or a directory containing .pth checkpoints",
    )
    parser.add_argument(
        "--model-class",
        default="rfdetr.RFDETRMedium",
        help="RF-DETR class path, e.g. rfdetr.RFDETRMedium",
    )
    parser.add_argument("--conf", default=0.01, type=float, help="RF-DETR detection threshold")
    parser.add_argument(
        "--rfdetr-classes",
        nargs="+",
        type=int,
        default=None,
        help="RF-DETR class ids to keep; omit to keep all detections",
    )
...
def compare_dataframes(gts, ts):
    accs = []
    names = []
    for key, tsacc in ts.items():
        if key in gts:
            logger.info("Comparing {}...".format(key))
            accs.append(mm.utils.compare_to_groundtruth(gts[key], tsacc, "iou", distth=0.5))
            names.append(key)
        else:
            logger.warning("No ground truth for {}, skipping.".format(key))
    return accs, names
...
def checkpoint_paths(ckpt):
    path = Path(ckpt)
    if path.is_file():
        return [path]
    if not path.is_dir():
        raise FileNotFoundError("Checkpoint path not found: {}".format(path))

    paths = sorted(path.glob("*.pth"))
    if not paths:
        raise FileNotFoundError("No RF-DETR .pth checkpoints found in: {}".format(path))
    return paths


def rfdetr_dets(detections, class_filter):
    ...
    if class_filter is not None:
        cls = np.asarray(detections.class_id, dtype=np.int64)
        keep = np.isin(cls, np.asarray(class_filter, dtype=np.int64))
        xyxy = xyxy[keep]
        conf = conf[keep]
...
def detection_summary(dataloader, data_list, inference_time, track_time, n_samples):
    ...
    info = ", ".join(
        [
            "Average {} time: {:.2f} ms".format(key, value)
...
def evaluate_rfdetr(exp, args, checkpoint_path, experiment_name):
    if args.batch_size != 1:
        raise ValueError("RF-DETR eval currently expects `-b/--batch-size 1` so sequences stay ordered.")
    ...
    logger.info("RF-DETR checkpoint: {}".format(checkpoint_path))

    val_loader = exp.get_eval_loader(args.batch_size, False, False)
    model_class = load_model_class(args.model_class)
    model = model_class().from_checkpoint(str(checkpoint_path))
    ...
    for cur_iter, (_, _, info_imgs, ids) in enumerate(tqdm(val_loader)):
        ...
        start = time.time()
        detections = model.predict(img_path, threshold=args.conf)
        infer_end = time.time()
        dets = rfdetr_dets(detections, args.rfdetr_classes)
...
def experiment_name_for_checkpoint(base_name, checkpoint_path, multiple):
    if multiple:
        return "{}_rfdetr_{}".format(base_name, checkpoint_path.stem)
    return "{}_rfdetr".format(base_name)
...
if __name__ == "__main__":
    ...
    if args.devices is not None and args.devices > 1:
        raise ValueError("tools/track_rfdetr.py currently supports single-device evaluation only.")

    checkpoints = checkpoint_paths(args.ckpt)
    multiple = len(checkpoints) > 1
    base_name = args.experiment_name or (exp.exp_name if exp.exp_name else "rfdetr_bytetrack")
    base_track_thresh = args.track_thresh
    base_track_buffer = args.track_buffer

    for checkpoint_path in checkpoints:
        args.track_thresh = base_track_thresh
        args.track_buffer = base_track_buffer
        experiment_name = experiment_name_for_checkpoint(base_name, checkpoint_path, multiple)
        results_folder = evaluate_rfdetr(exp, args, checkpoint_path, experiment_name)
